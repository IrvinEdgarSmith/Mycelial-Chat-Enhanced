...
import { googleSearch, formatSearchResultsAsContext } from '@/services/googleSearchService';
import {
  Command,
  CommandDialog,
  CommandEmpty,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      let systemPrompt =
        ...
        settings.defaultSystemPrompt || // Use defaultSystemPrompt from settings
      ...
      if (isWebSearchEnabled) {
        if (!settings.googleSearchApiKey || !settings.googleSearchId) {
          toast({
            title: 'Error',
            description: 'Google Search API Key and Search ID are required for web search.',
            variant: 'destructive',
            duration: 3000,
          });
          setIsLoading(false);
        ...
        try {
          const searchResults = await googleSearch(userMessage, settings.googleSearchApiKey, settings.googleSearchId);
          const formattedContext = formatSearchResultsAsContext(searchResults, userMessage);
          systemPrompt = `${systemPrompt}\n\n${formattedContext}`;
        } catch (searchError) {
          console.error('Error performing web search:', searchError);
          toast({
            title: 'Web Search Error',
            description: 'Failed to perform web search. Please check your API keys and try again.',
            variant: 'destructive',
            duration: 3000,
          });
        }
        ...
        >
          ...
          <button
            ...
            className={`h-8 w-8 rounded-md border border-gray-400 mr-2 transition-colors duration-200 flex items-center justify-center ${
              isWebSearchEnabled ? 'bg-green-500' : 'bg-transparent'
            }`}
            onClick={handleWebSearchToggle}
          >
            <Globe className={`h-4 w-4 ${isWebSearchEnabled ? 'text-white' : 'text-gray-400'}`} />
